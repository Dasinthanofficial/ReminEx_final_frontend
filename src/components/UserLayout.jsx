...
import { useAuth } from '../context/AuthContext';
import {
 FiHome, FiBox, FiPlusCircle, FiUser, FiLogOut, FiCreditCard, FiArrowLeft, FiMenu, FiX
} from 'react-icons/fi';
   ...
   <div className="flex h-screen bg-[#122017] font-sans selection:bg-[#38E07B] selection:text-[#122017] overflow-hidden relative">
     ...
     <AnimatePresence>
        {isMobileMenuOpen && (
            <motion.div 
                ...
                animate={{ x: 0 }}
                exit={{ x: '-100%' }}
                className="fixed inset-0 z-30 bg-[#122017] md:hidden pt-20 px-6 flex flex-col h-full"
            >
                    ...
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            `flex items-center px-4 py-4 rounded-xl transition-all ${
                                isActive ? 'bg-[#38E07B] text-[#122017] font-bold' : 'text-gray-400 hover:bg-white/5'
                            }`
                            }
                        >
     ...
     <aside className="hidden md:flex w-72 flex-col fixed h-full z-20 border-r border-white/10 bg-black/20 backdrop-blur-2xl">
       ...
       <div className="p-6 border-t border-white/5">
         <button onClick={handleLogout} className="flex items-center justify-center w-full px-4 py-3 text-red-400 bg-red-500/10 rounded-xl hover:bg-red-500/20 transition-all font-bold text-sm">
     ...
     {/* 🧱 Main Content Area */}
     <main className="flex-1 md:ml-72 h-screen overflow-y-auto relative z-10 custom-scrollbar pt-16 md:pt-0">
       <div className="max-w-7xl mx-auto p-4 md:p-8 lg:p-10 pb-24 md:pb-10">
          <Outlet />
       </div>
     </main>
   </div>
