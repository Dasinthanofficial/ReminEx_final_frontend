...
const AddProduct = () => {
  ...
  // Scanner state
  const [showScanner, setShowScanner] = useState(false);
  const [scanning, setScanning] = useState(false);        // barcode
  ...
  // Image upload handler (used for product image & label OCR)
  const handleFile = (e) => {
    const f = e.target.files[0];
    if (f) {
      if (!f.type.startsWith("image/")) {
  ...
  // Auto-fill using backend (barcode)
  const handleAutoFill = async (code) => {
    const trimmed = (code || "").trim();
    if (!trimmed) {
      toast.error("Please enter or scan a barcode first.");
  ...
  // 🔍 Scan label with OCR to guess expiry / weight
