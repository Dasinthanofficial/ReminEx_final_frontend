...
const ManagePlans = () => {
  ...
  const handleInitDefaults = async () => {
    ...
    try {
      for (const p of defaults) await api.post("/plans", p);
      queryClient.invalidateQueries({ queryKey: ["plans"] });
      toast.success("Default plans created!");
    } catch (e) {
      toast.error(e?.response?.data?.message || "Failed to seed plans");
  ...
  const handleSave = () => {
    ...
    else updateMutation.mutate({ id: editingPlan, data: payload });
  };
  ...
  const renderForm = () => (
    <div className="space-y-4 text-white min-w-0">
      ...
      <div>
        ...
        />
        {formData.name === "Free" && (
          <p className="text-[10px] text-gray-500 mt-1">Free plan price is always $0.</p>
        )}
      </div>
      ...
      <div>
        ...
        {formData.features.map((feat, idx) => (
          <div key={idx} className="flex gap-2 mb-2 min-w-0">
            <input
              value={feat}
              onChange={(e) => handleFeatureChange(idx, e.target.value)}
              className="flex-1 min-w-0 bg-black/40 border border-white/10 rounded-xl p-2 text-sm text-white focus:border-[#38E07B] outline-none"
              placeholder="e.g. Unlimited Products"
            />
            <button
              type="button"
              onClick={() => removeFeatureField(idx)}
              className="text-red-400 hover:text-red-500 p-2 shrink-0"
              title="Remove feature"
            >
              <FiX />
            </button>
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-4">
        <button
          ...
          onClick={handleSave}
          disabled={savingNow}
          className="w-full sm:flex-1 bg-[#38E07B] text-[#122017] font-bold py-2 rounded-xl hover:bg-[#2fc468] transition flex items-center justify-center gap-2 disabled:opacity-60"
        >
          <FiSave /> {savingNow ? "Saving..." : "Save"}
        </button>
        ...
        <button
          ...
          className="w-full sm:flex-1 bg-white/10 text-white font-bold py-2 rounded-xl hover:bg-white/20 transition"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <AdminPageShell>
      <div className="space-y-8 pb-12 min-w-0">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 min-w-0">
          ...
          <button
            type="button"
            onClick={startCreate}
            className="w-full sm:w-auto bg-[#38E07B] text-[#122017] px-5 py-2.5 rounded-xl font-bold shadow-lg hover:bg-[#2fc468] transition flex items-center justify-center gap-2 text-sm"
          >
            <FiPlus size={18} /> New Plan
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 min-w-0">
          {isCreating && (
            <div className="bg-white/5 backdrop-blur-xl p-6 rounded-3xl border-2 border-[#38E07B] relative shadow-2xl min-w-0 overflow-hidden">
              <div className="absolute -top-3 left-6 bg-[#38E07B] text-[#122017] text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                Creating New
              </div>
              {renderForm()}
          ...
          {plans?.map((plan) => {
            const isEditingThis = editingPlan === plan._id;
            const canDelete = plan.name !== "Free";
            const priceSuffix = plan.name === "Free" ? "" : plan.name === "Yearly" ? "/yr" : "/mo";

            return (
              <div
                key={plan._id}
                className={`bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 shadow-lg relative transition-all min-w-0 overflow-hidden ${
                  isEditingThis ? "ring-2 ring-[#38E07B]" : "hover:bg-white/10"
                }`}
              >
                {isEditingThis ? (
                  ...
                  <>
                    <div className="flex justify-between items-start mb-4 gap-3 min-w-0">
                      <div className="min-w-0">
                        ...
                        <div className="text-2xl font-bold text-[#38E07B] mt-1 break-words">
                          ${plan.price}
                          <span className="text-sm text-gray-400 font-normal">
                            {priceSuffix}
                          </span>
                      ...
                      <div className="flex gap-2 shrink-0">
                        ...
                        <button
                          ...
                          disabled={!canDelete || deleteMutation.isPending}
                          onClick={() => {
                            if (!canDelete) return toast.error("Free plan should not be deleted.");
                            if (window.confirm(`Delete ${plan.name} plan?`)) deleteMutation.mutate(plan._id);
                          }}
                          className={`p-2 bg-white/5 rounded-lg transition ${
                            canDelete ? "text-gray-400 hover:text-red-400" : "text-gray-600 cursor-not-allowed opacity-60"
                          }`}
                          title={canDelete ? "Delete plan" : "Free plan cannot be deleted"}
                        >
                          <FiTrash2 />
                    ...
                    <p className="text-gray-400 text-sm mb-6 min-h-[40px] leading-relaxed break-words">
                      {plan.description}
                    </p>

                    <div className="space-y-3 border-t border-white/10 pt-4">
                      {plan.features?.map((feat, i) => (
                        <div key={i} className="flex items-start text-sm text-gray-300 break-words">
                          <FiCheck className="text-[#38E07B] mr-2 mt-0.5 flex-shrink-0" />
