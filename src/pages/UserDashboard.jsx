...
import { useQuery } from '@tanstack/react-query';
import { motion } from 'framer-motion';
import { FiPackage, FiAlertTriangle, FiDollarSign, FiPlus, FiChevronRight, FiShield, FiEdit3 } from 'react-icons/fi';
import { format } from 'date-fns';
import { productService } from '../services/productService';
import { useAuth } from '../context/AuthContext';
import RecipeSuggestions from '../components/RecipeSuggestions';
...
const UserDashboard = () => {
  ...
  // Admin view (No changes needed here)
  if (user?.role === 'admin') {
    return (
      <div className="space-y-8">
        {/* Welcome Section */}
  ...
  // Calculate statistics for regular users (remains the same)
  const stats = {
    totalProducts: products?.length || 0,
    expiringThisWeek: products?.filter(p => {
      const daysUntilExpiry = Math.ceil((new Date(p.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
  ...
  return (
    <div className="space-y-8">
      {/* 🟢 FIXED: Welcome Section with Avatar and Edit Link */}
      <div className="bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-2xl p-8">
        <div className="flex justify-between items-start">
          ...
          {/* Edit Profile Button */}
          <Link
            to="/profile" // ⚠️ Requires a new /profile route in your router
      ...
      {/* ... (rest of the dashboard: recent products, expiring soon, reports, suggestions) ... */}
