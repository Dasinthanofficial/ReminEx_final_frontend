...
//       startOfDay(new Date(expiryDate)),
//       startOfDay(new Date())
//     );
...
//   if (user?.role === "admin") {
//     return <div className="p-8 text-center text-white">Admin restricted.</div>;
//   }
...
const Products = () => {
  ...
  // Admins should never be here because PrivateRoute redirects them,
  // but keeping this guard is fine.
  if (isAdminRole) {
    return <div className="p-8 text-center text-white">Admin restricted.</div>;
  }
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a2c23] border border-white/10 rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center transform transition-all scale-100 relative overflow-hidden">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={() => productToDelete?._id && deleteMutation.mutate(productToDelete._id)}
                disabled={deleteMutation.isPending || !productToDelete?._id}
                className="flex-1 py-2.5 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 transition disabled:opacity-50 text-sm shadow-lg shadow-red-900/20"
              >
                {deleteMutation.isPending ? "Deleting..." : "Delete"}
              </button>
