...
//     if (days < 0) return { text: 'Expired', className: 'bg-red-500/20 text-red-400 border-red-500/30' };
//     if (days === 0) return { text: 'Expires Today', className: 'bg-red-500/20 text-red-400 border-red-500/30' };
//     if (days <= 3) return { text: `${days} Days Left`, className: 'bg-orange-500/20 text-orange-400 border-orange-500/30' };
//     if (days <= 7) return { text: `${days} Days Left`, className: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' };
//     return { text: 'Fresh', className: 'bg-green-500/20 text-green-400 border-green-500/30' };
//   };
...
//   if (user?.role === 'admin') return <div className="p-8 text-center text-white">Admin restricted.</div>;
...
//         <div>
//           <h1 className="text-3xl font-bold text-white tracking-tight">My Inventory</h1>
//           <p className="text-gray-400 mt-1 text-sm">Manage your food and household items.</p>
//         </div>
...
//             return (
//               <div key={product._id} className="group bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition-all duration-300 overflow-hidden flex flex-col hover:-translate-y-1">
                
//                 {/* Card Image */}
//                 <div className="relative h-48 bg-black/20 overflow-hidden">
//                    {imgSrc ? (
//                      <img src={imgSrc} alt={product.name} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
//                    ) : (
...
//                    {/* Status Badge */}
//                    <div className={`absolute top-3 right-3 px-3 py-1 rounded-full text-[10px] font-bold border shadow-sm backdrop-blur-md ${status.className}`}>
//                      {status.text}
//                    </div>
//                 </div>
...
//                     <div>
//                       <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{product.category}</p>
...
//           <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl">🤷‍♂️</div>
//           <h3 className="text-xl font-bold text-white mb-2">No products found</h3>
//           <p className="text-gray-400 mb-6">Try adjusting your search or add a new item.</p>
//           <Link to="/products/add" className="inline-flex items-center gap-2 text-[#38E07B] font-bold hover:underline">
...
//               Are you sure you want to remove <strong className="text-white">{productToDelete?.name}</strong>? This action cannot be undone.
//             </p>
              ...
              <div key={product._id} className="group bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition-all duration-300 overflow-hidden flex flex-col hover:-translate-y-1">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 space-y-2 border-t border-white/5">
                    ...
                    {/* 👇 UPDATED: Shows Weight + Unit (e.g. 500 ml) */}
                    {product.weight && (
                       <div className="flex items-center text-sm text-gray-400">
                         <FiArchive className="mr-2" />
                         <span>{product.weight} {product.unit || 'g'}</span>
                       </div>
                    )}
                  </div>
